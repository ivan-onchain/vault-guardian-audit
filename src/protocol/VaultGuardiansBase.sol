...
contract VaultGuardiansBase is AStaticTokenData, IVaultData {
     ...
     */
    // Medium There is NOT GUARDIAN_FEE requirement to become guardian. 
    // This will happen always and the functionality doesn't reflect the intention of the developer.
        ...
        returns (address)
    {
        ...
        if (address(token) == address(i_tokenOne)) {
            ...
            new VaultShares(IVaultShares.ConstructorData({
                asset: token,
                // Wrong vaultName and vaultSymbol
                // High: By error somebody could send the wrong token
                vaultName: TOKEN_ONE_VAULT_NAME,
                vaultSymbol: TOKEN_ONE_VAULT_SYMBOL,
                guardian: msg.sender,
                allocationData: allocationData,
     ...
     */
    // q what if the tokenVault is a vault configured with a different token, I mean wethVault and USDC or wbtc
    // r it should be allowed because those tokens where allowed for a trusted owner.
    function _becomeTokenGuardian(IERC20 token, VaultShares tokenVault) private returns (address) {
        s_guardians[msg.sender][token] = IVaultShares(address(tokenVault));
        emit GuardianAdded(msg.sender, token);
        //HIGH: As guardian can becomeTokenGuardian and quitGuardian w/out restrictions he can mint unlimited vgToken 
        // to gain power voting 
