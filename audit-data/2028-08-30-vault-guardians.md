...
### [M-2] `VaultGuardiansBase::becomeGuardian` doesn't charge users with a ETH amount fee to be guardians.

`VaultGuardiansBase::becomeGuardian` natspec claims that ***they have to send an ETH amount equal to the fee***
but the code doesn't require this fee as a charge despite of the `GUARDIAN_FEE` constant exists a the beggining of the code, meaning that really it was an intention to charge users with ETH to be guardians.

Definitely this restriction should be added to help to mitigate some of the vulnerabilities related with indefinitely  vgToken farming.  
...
### [L-3] `VaultGuardians::updateGuardianAndDaoCut` emits an event that doesn't reflect the function action.

`VaultGuardians__UpdatedStakePrice` doesn't reflect the `updateGuardianAndDaoCut` actions, this could lead to confusion for off-chain data consumers.

Consider create the right and add event.
```js
...
### [L-4] `VaultGuardians::sweepErc20s` function should be restricted to onlyOwners.

This function should be restricted by onlyOwner to avoid miss handling.
...
### [I-1] `AStaticWethData` inaccurate contract comments. 

Comments talk about 4 tokens but that is not reflected in that abstract contract code. This may confuse to the readers.
```js
...
### [I-2] `IInvestableUniverseAdapter` Interface is not used.

Delete unused code.

### [I-3] `VaultGuardians__UpdatedFee` event in `VaultGuardians` contract is not used.

Verify but this event is not used, maybe a updateFee function was planned ahead and developers missed add it?